...
public:
    ...
    // duck-typed void emitWData(vluint32_t code, const WDataP newvalp, int bits) = 0;
    ...
    void fullWData(vluint32_t* oldp, WDataP newvalp, int bits);
    ...
    inline void chgWData(vluint32_t code, WDataP newvalp, int bits) {
        m_traceBufferWritep[0] = (bits << 4) | VerilatedTraceCommand::CHG_WDATA;
        m_traceBufferWritep[1] = code;
        m_traceBufferWritep += 2;
        for (int i = 0; i < (bits + 31) / 32; ++i) { *m_traceBufferWritep++ = newvalp[i]; }
...
#define CHG(name) chg##name
    ...
    inline void CHG(WData)(vluint32_t* oldp, WDataP newvalp, int bits) {
        for (int i = 0; i < (bits + 31) / 32; ++i) {
            if (VL_UNLIKELY(oldp[i] ^ newvalp[i])) {
                fullWData(oldp, newvalp, bits);
                return;
